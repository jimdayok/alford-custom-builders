...
export async function fetchPublishedRows(contentType: string, contentKey: string) {
  ...
  const response = await fetch(`${url}/rest/v1/rpc/get_published_content`, {
    ...
    body: JSON.stringify({ requested_site_slug: SITE_SLUG, requested_content_type: contentType, requested_content_key: contentKey }),
    next: { tags: [`site:${SITE_SLUG}`, `content:${contentType}`, `content:${contentType}:${contentKey}`] },
  });
